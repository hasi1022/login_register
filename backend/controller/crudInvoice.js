...
export const getInvoiceUpdate = async (req,res)=>{
  ...
  const invoiceUpdate=await invoice.findOne({where:{invoiceId:invoiceId},include:[{
    model:items,
    as:'items'
  }]})
...
export const updateInvoice = async (req,res) =>{
    ...
    await items.destroy({
      where:{invoiceId:update_id},
      itemId:{[Op.notIn]:updatedItem}
    })
    const ress=await items.bulkCreate(item,{updateOnDuplicate:['itemName','itemQuantity', 'itemUnitPrice', 'itemSubUnitTotal', 'itemGstPer','itemGst']})
